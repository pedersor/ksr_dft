...
def lsda_scf_example():
  ...
  system_info = scf.KohnShamState(density=ref_density,
                                  total_energy=ref_total_energy,
                                  external_potential=external_potentials,
                                  nuclear_charges=nuclear_charges,
                                  grids=np.tile(np.expand_dims(grids, axis=0),
                                                reps=(num_samples, 1)),
  ...
  for i in range(len(system_info.num_electrons)):
    ...
    states = {
        "spin_scf result: ": lsda_ksdft_scf,
        "jit_spin_scf result: ": lsda_ksdft_jit_scf
    }
    for label, state in states.items():
      ...
      np.testing.assert_allclose(system_info.total_energy[i],
                                 total_energy,
                                 rtol=0,
                                 atol=10**(-3))
      np.testing.assert_allclose(system_info.xc_energy[i],
                                 xc_energy,
                                 rtol=0,
                                 atol=10**(-3))
...
def lda_scf_example():
  ...
  for i in range(len(system_info.num_electrons)):
    ...
    print(f'Example system: {symbols[i]}')
    states = {
        "scf result: ": lda_ksdft_scf,
        "jit_scf result: ": lda_ksdft_jit_scf
    }
    for label, state in states.items():
      total_energy = state.total_energy[-1].block_until_ready()
      kinetic_energy = state.kinetic_energy[-1]
