...
training_set_1 = h2_dataset.get_molecules(to_train[0])
training_set_2 = h3_dataset.get_molecules(to_train[1])
training_set = datasets.concatenate_kohn_sham_states(training_set_1,
  training_set_2)
trainer.set_training_set(training_set)
print(f'to_train = {to_train}')

validation_sets_dict = {'v2': [[168], [224]]}
# load validation set from sys passed arg
validation_dir = sys.argv[2]
to_validate = validation_sets_dict[validation_dir]

validation_set_1 = h2_dataset.get_molecules(to_validate[0])
validation_set_2 = h3_dataset.get_molecules(to_validate[1])
validation_set = datasets.concatenate_kohn_sham_states(validation_set_1,
  validation_set_2)
...
trainer.set_ks_params(  # The number of Kohn-Sham iterations in training.
  num_iterations=30,  # @The density linear mixing factor.
...
trainer.setup_optimization(initial_checkpoint_index=0, save_every_n=10,
  max_train_steps=500, num_skipped_energies=-1, )
