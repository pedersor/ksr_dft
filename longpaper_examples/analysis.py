...
def get_error_table(test_dataset, final_states):
  ...
  for i in range(len(test_dataset.total_energies)):
    ...
    density_loss = np.mean(
      (ksr_density - exact_density) ** 2 / test_dataset.num_electrons[
        i]) * utils.get_dx(test_dataset.grids) * len(test_dataset.grids)
...
def get_ip_table(test_dataset, final_states,
    ions_idx_to_compare=np.array([-1, -1, -1, -1, 1, 2, 3, 5, 6, 8])):
  ...
  for i in range(len(test_dataset.total_energies)):
    ...
    if ions_idx_to_compare[i] == -1:
      ...
      ksr_ip = np.abs(
        final_states.total_energy[i] - final_states.total_energy[idx])
      exact_ip = np.abs(
        test_dataset.total_energies[i] - test_dataset.total_energies[idx])
