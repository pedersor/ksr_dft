...
def _solve_noninteracting_system(external_potential, num_electrons, grids):
  ...
  density = wavefunctions_to_density(num_electrons,
                                     jnp.transpose(wavefunctions_transpose),
                                     grids)
  total_eigen_energies = get_total_eigen_energies(num_electrons=num_electrons,
...
def batch_solve_noninteracting_system(external_potential, num_electrons, grids):
  return jax.vmap(solve_noninteracting_system,
                  in_axes=(0, 0, None),
...
def get_xc_potential_sigma(densities, xc_energy_density_fn, grids):
  ...
  xc_potentials = jax.grad(get_xc_energy_sigma)(densities, xc_energy_density_fn,
                                                grids)
  xc_potentials = jnp.nan_to_num(
      jnp.asarray(xc_potentials)) / utils.get_dx(grids)
...
def kohn_sham_iteration(state, xc_energy_density_fn, interaction_fn,
  ...
  hartree_potential = scf.get_hartree_potential(density=state.density,
                                                grids=state.grids,
  ...
  xc_potential_up, xc_potential_down = get_xc_potential_sigma(
      densities, xc_energy_density_fn, state.grids)
  ...
  densities, total_eigen_energies_sigma = (batch_solve_noninteracting_system(
      ks_potentials_sigma, num_electrons_sigma, state.grids))
  ...
  for i, (density_sigma, total_eigen_energy_sigma,
          ks_potential_sigma) in enumerate(
              zip(densities, total_eigen_energies_sigma, ks_potentials_sigma)):
    density += density_sigma
    spin_density += (-1)**(i) * density_sigma
    kinetic_energy += (
        total_eigen_energy_sigma -
        scf.get_external_potential_energy(external_potential=ks_potential_sigma,
                                          density=density_sigma,
                                          grids=state.grids))
  ...
  # xc energy
  xc_energy = get_xc_energy_sigma(densities,
...
def kohn_sham(locations,
  ...
  state = scf.KohnShamState(density=initial_density,
  ...
  for _ in range(num_iterations):
    ...
    if density_diffs is None and spin_density_diffs is None:
      ...
      spin_density_diffs = jnp.vstack([spin_density_diffs, spin_density_diff])
    if jnp.mean(jnp.square(density_diff)) < density_mse_converge_tolerance:
      converged = True
