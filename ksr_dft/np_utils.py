...
def _get_exact_h_atom_density(displacements, dx, energy=-0.670):
  ...
  if displacements.ndim != 2:
    raise ValueError('displacements is expected to have ndim=2, but got %d' %
                     displacements.ndim)
  # Defined after Equation 2.
  v = np.sqrt(-8 * energy / constants.EXPONENTIAL_COULOMB_KAPPA**2)
  # Equation 2.
  z = (2 * np.sqrt(2 * constants.EXPONENTIAL_COULOMB_AMPLITUDE) /
       constants.EXPONENTIAL_COULOMB_KAPPA *
       np.exp(-constants.EXPONENTIAL_COULOMB_KAPPA * np.abs(displacements) / 2))
  # Equation 3.
  raw_exact_density = special.jv(v, z)**2
  return raw_exact_density / (np.sum(raw_exact_density, axis=1, keepdims=True) *
                              dx)
