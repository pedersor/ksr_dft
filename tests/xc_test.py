...
class XcTest(parameterized.TestCase):
  ...
  def test_exponential_coulomb_uniform_exchange_density(self):
    np.testing.assert_allclose(
        xc.unpolarized_exponential_coulomb_uniform_exchange_density(
            density=jnp.array([1e-15, 1e-10, 1e-5, 1., 2., 3., 20.])), [
                0., 0., -0.000013, -0.3983580525, -0.4512822906, -0.4732600516,
                -0.521973703
            ],
        atol=1e-6)
  ...
  def test_exponential_coulomb_uniform_exchange_density_high_density_limit(
    ...
    np.testing.assert_allclose(
        xc.unpolarized_exponential_coulomb_uniform_exchange_density(
            density=jnp.array([1000., 10000.])), [limit_value, limit_value],
        atol=1e-3)
  ...
  def test_exponential_coulomb_uniform_correlation_density_low_density_limit(
    ...
    beta = -1.00077
    np.testing.assert_allclose(
        xc.unpolarized_exponential_coulomb_uniform_correlation_density(
            density=density),
  ...
  def test_exponential_coulomb_uniform_correlation_density_high_density_limit(
    ...
    np.testing.assert_allclose(
        xc.unpolarized_exponential_coulomb_uniform_correlation_density(
            density=jnp.array([1000., 10000.])), [0., 0.],
        atol=1e-3)
  ...
  def test_exponential_coulomb_uniform_correlation_density_gradient(self):
    ...
    sigma = -1.48334
    nu = 1.
    denominator = (alpha + beta * jnp.sqrt(y) + gamma * y +
                   delta * jnp.sqrt(y**3) + eta * y**2 +
                   sigma * jnp.sqrt(y**5) +
                   nu * jnp.pi * self.kappa**2 / self.amplitude * y**3)
    y_ddenominator = (beta * 0.5 * y**0.5 + gamma * y + delta * 1.5 * y**1.5 +
    ...
    self.assertAlmostEqual(
        jax.grad(
            xc.unpolarized_exponential_coulomb_uniform_correlation_density)(0.),
  ...
  def test_get_lda_xc_energy_density_fn(self, params):
    ...
    np.testing.assert_allclose(lda_xc_energy_density_fn(
        density=jnp.array([0., 1.]), params=params), [0., -0.406069],
                               atol=1e-5)
