...
class UtilsTest(parameterized.TestCase):
  ...
  def test_soft_coulomb(self, center):
    ...
    self.assertAlmostEqual(float(grids[jnp.argmax(soft_coulomb_interaction)]),
                           center)
  ...
  def test_exponential_coulomb(self, center):
    ...
    self.assertAlmostEqual(float(jnp.amax(soft_coulomb_interaction)),
  ...
  @parameterized.parameters(
      ([[-0.1], [0.], [0.1]
       ], [1, 3], [1, 2], 'grids.ndim is expected to be 1 but got 2'),
      ([-0.1, 0., 0.1
       ], [[1], [3]], [1, 2], 'locations.ndim is expected to be 1 but got 2'),
      ([-0.1, 0., 0.1], [1, 3], [[1], [2]],
       'nuclear_charges.ndim is expected to be 1 but got 2'),
  )
  def test_get_atomic_chain_potential_incorrect_ndim(self, grids, locations,
                                                     nuclear_charges,
                                                     expected_message):
  ...
  def test_get_nuclear_interaction_energy(self, locations, nuclear_charges,
                                          interaction_fn, ecpected_energy):
    self.assertAlmostEqual(
        float(
            utils.get_nuclear_interaction_energy(
                locations=jnp.array(locations),
                nuclear_charges=jnp.array(nuclear_charges),
                interaction_fn=interaction_fn)), ecpected_energy)
  ...
  def test_get_nuclear_interaction_energy_batch(self, locations,
    ...
    np.testing.assert_allclose(
        utils.get_nuclear_interaction_energy_batch(
            locations=jnp.array(locations),
            nuclear_charges=jnp.array(nuclear_charges),
            interaction_fn=interaction_fn), ecpected_energies)
  ...
  def test_float_value_in_array_true(self, value):
    self.assertTrue(
        utils._float_value_in_array(value,
                                    array=jnp.array([-0.1, 0.0, 0.1, 0.2,
                                                     0.3])))
  ...
  def test_compute_distances_between_nuclei(self):
    np.testing.assert_allclose(
        utils.compute_distances_between_nuclei(locations=np.array([
            [-1., 1., 3.5, 5.],
            [-4., 0., 3.5, 10.],
            [-2., -1., 3.5, 55.],
        ]),
